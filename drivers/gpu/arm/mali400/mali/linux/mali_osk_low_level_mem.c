...
static unsigned long mali_mem_shrink_count(struct shrinker *shrinker, struct shrink_control *sc)
{
...
static int mali_mem_shrink(struct shrinker *shrinker, struct shrink_control *sc)
	#else
static unsigned long mali_mem_shrink(struct shrinker *shrinker, struct shrink_control *sc)
	#endif
#endif
{
	unsigned long flags;
	...
	if (0 == nr)
	{
		return mali_mem_shrink_count(shrinker, sc);
	}
