...
int mali_platform_device_register(void)
{
	...
	if (mali_read_phys(0x6F000000) == 0x40601450) {
		MALI_PRINT(("Registering Mali-450 MP6 device\n"));
		num_pp_cores = 6;
	...
	if (mali_read_phys(0xFC000000) == 0x00000450) {
		MALI_PRINT(("Registering Mali-450 MP8 device\n"));
		num_pp_cores = 8;
		...
		MALI_PRINT(("Registering Mali-450 MP4 device\n"));
		num_pp_cores = 4;
	...
	if ((m400_gp_version & 0xFFFF0000) == 0x0C070000) {
		MALI_PRINT(("Registering Mali-300 device\n"));
		num_pp_cores = 1;
		...
		if (fpga_fw_version == 0x130C008F || fpga_fw_version == 0x110C008F) {
			...
			MALI_PRINT(("Registering Mali-400 MP1 device\n"));
			num_pp_cores = 1;
			...
			/* Mali-400 MP2 r1p1 */
			MALI_PRINT(("Registering Mali-400 MP2 device\n"));
			num_pp_cores = 2;
...
{
	MALI_PRINT(("mali_platform_device_unregister() called\n"));
...
static void mali_platform_device_release(struct device *device)
{
	MALI_PRINT(("mali_platform_device_release() called\n"));
}
...
int mali_platform_device_deinit(struct platform_device *device)
{
	...
	MALI_PRINT(("mali_platform_device_deinit() called\n"));
