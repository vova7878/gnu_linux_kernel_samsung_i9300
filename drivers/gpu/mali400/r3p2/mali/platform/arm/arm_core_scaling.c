...
static void enable_one_core(void)
{
	if (num_cores_enabled < num_cores_total) {
		...
		MALI_PRINT(("Core scaling: Enabling one more core\n"));
	}
...
static void disable_one_core(void)
{
	if (1 < num_cores_enabled) {
		--num_cores_enabled;
		schedule_work(&wq_work);
		MALI_PRINT(("Core scaling: Disabling one core\n"));
	}
...
static void enable_max_num_cores(void)
{
	if (num_cores_enabled < num_cores_total) {
		...
		schedule_work(&wq_work);
		MALI_PRINT(("Core scaling: Enabling maximum number of cores\n"));
	}
...
void mali_core_scaling_update(struct mali_gpu_utilization_data *data)
{
	...
	MALI_PRINT(("Utilization: (%3d, %3d, %3d), cores enabled: %d/%d\n", data->utilization_gpu, data->utilization_gp, data->utilization_pp, num_cores_enabled, num_cores_total));
