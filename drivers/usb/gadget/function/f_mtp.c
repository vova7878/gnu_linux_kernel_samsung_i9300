 ...
 * f_mtp.c -- MTP Driver, for MTP development,
 *
...
/*
 * f_mtp_slp.c file is the driver for MTP device. Totally three
...
#define MTPG_MTPG_TX_REQ_MAX		 10
#define MTPG_INTR_REQ_MAX		 2
...
static struct usb_endpoint_descriptor int_fs_notify_desc = {
	...
	.wMaxPacketSize	= cpu_to_le16(MTPG_INTR_BUFFER_SIZE),
	.bInterval =		6,
};
...
static struct usb_ss_ep_comp_descriptor ss_intr_notify_desc = {
	...
	.bDescriptorType =	USB_DT_SS_ENDPOINT_COMP,
	.wBytesPerInterval =	cpu_to_le16(MTPG_INTR_BUFFER_SIZE),
};
...
struct {
	...
	.header = {
		.dwLength = cpu_to_le32(sizeof(mtpg_ext_config_desc)),
...
static int mtp_send_signal(int value)
{
	...
	if (ret < 0) {
		DEBUG_MTPB("[%s]error sending signal\n", __func__);
		return ret;
	}
	return 0;
...
static int mtpg_open(struct inode *ip, struct file *fp)
{
	if (_lock(&the_mtpg->open_excl)) {
		DEBUG_MTPB("mtpg_open fn mtpg device busy\n");
		return -EBUSY;
	}
				...
				size_t count, loff_t *pos)
{
	...
	while (!((dev->online || dev->error) && dev->read_ready)) {
		...
		if (ret < 0) {
			...
			DEBUG_MTPR("[%s]line is = %d,mtp_read ret<0\n",
							__func__, __LINE__);
			return ret;
		}
	}
	...
	while (count > 0) {
		...
		if (dev->error) {
			r = -EIO;
			DEBUG_MTPB("[%s]\t%d:dev->error so break r=%d\n",
						__func__, __LINE__, r);
			break;
		}
...
requeue_req:
		...
		if (dev->read_count > 0) {
			...
			if (copy_to_user(buf, dev->read_buf, xfer)) {
				r = -EFAULT;
				DEBUG_MTPR("[%s]%d:cpytouer fail r=%d\n",
						__func__, __LINE__, r);
				break;
			}
		...
		if (ret < 0) {
			r = ret;
			DEBUG_MTPR("[%s]\t%d after ret=%d brk ret=%d\n",
				 ...
				 size_t count, loff_t *pos)
{
	...
	while (count > 0) {
		if (dev->error) {
			r = -EIO;
			DEBUG_MTPW("[%s]%d count>0 dev->error so brk\n",
					__func__, __LINE__);
			break;
		}
		...
		if (req != 0) {
			...
			if (copy_from_user(req->buf, buf, xfer)) {
				DEBUG_MTPW("mtpwrite cpyfrmusr error\n");
				r = -EFAULT;
				break;
			}
...
static void read_send_work(struct work_struct *work)
{
	...
	DEBUG_MTPB("[%s:%d] offset=[%lld]\t leth+hder=[%lld]\n",
	...
	/* Make sure the result is stored to memory */
	smp_wmb();
}
...
static long mtpg_ioctl(struct file *fd, unsigned int code, unsigned long arg)
{
	...
	cdev = the_mtpg->function.config->cdev;
	req = cdev->req;
	bulk_in = the_mtpg->bulk_in;
	bulk_out = the_mtpg->bulk_out;
	...
	case GET_MAX_PKT_SIZE:
		/* Now at tx-case, we use variable size in udc-max-tx-pkt-size
		 * limitation to increase transferring perfomance.
	...
	case MTP_WRITE_INT_DATA:
		DEBUG_MTPB("[%s]\t%d MTP intrpt_Write no slep\n",
						__func__, __LINE__);
		if (copy_from_user(&event, (void __user *)arg, sizeof(event))) {
			status = -EFAULT;
			DEBUG_MTPB("[%s]\t%d:copyfrmuser fail\n",
							 __func__, __LINE__);
			break;
		}
		ret_value = interrupt_write(fd, &event,
				MTP_MAX_PACKET_LEN_FROM_APP);
		if (ret_value < 0) {
			DEBUG_MTPB("[%s]\t%d interptFD failed\n",
							 __func__, __LINE__);
			status = -EIO;
		} else {
			DEBUG_MTPB("[%s]\t%d intruptFD suces\n",
							 __func__, __LINE__);
	...
	case SET_ZLP_DATA:
		...
		req = mtpg_req_get(dev, &dev->tx_idle);
		if (!req) {
			DEBUG_MTPB("[%s] Failed to get ZLP_DATA\n", __func__);
			return -EAGAIN;
		}
		req->length = 0;
		DEBUG_MTPB("[%s]ZLP_DATA data=%d\tline=[%d]\n",
						 __func__, size, __LINE__);
		status = usb_ep_queue(dev->bulk_in, req, GFP_ATOMIC);
		if (status < 0) {
			DEBUG_MTPB("[%s]usbepqueue line=[%d]\n",
							 __func__, __LINE__);
		} else {
			DEBUG_MTPB("%sZLPstatus=%d\tline=%d\n",
...
#ifdef CONFIG_COMPAT
static long
mtpg_compat_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
...
static DEVICE_ATTR(guid,  S_IRUGO | S_IWUSR, guid_show, guid_store);
...
mtpg_function_bind(struct usb_configuration *c, struct usb_function *f)
{
	...
	if (id < 0) {
		DEBUG_MTPB("[%s]Error in usb_interface_id\n", __func__);
		return id;
	}
	...
	if (!ep) {
		DEBUG_MTPB("[%s]Error usb_ep_autoconfig IN\n", __func__);
		goto autoconf_fail;
	}
	...
	if (!ep) {
		DEBUG_MTPB("[%s]Error usb_ep_autoconfig OUT\n", __func__);
		goto autoconf_fail;
	}
	ep->driver_data = mtpg;		/* claim the endpoint */
	mtpg->bulk_out = ep;
	...
	if (!ep) {
		DEBUG_MTPB("[%s]Error usb_ep_autoconfig INT\n", __func__);
		goto autoconf_fail;
	}
	ep->driver_data = mtpg;
	mtpg->int_in = ep;
	...
	DEBUG_MTPB("%s speed %s: IN/%s, OUT/%s\n",
...
autoconf_fail:
	DEBUG_MTPB("mtpg unable to autoconfigure all endpoints\n");
	return -ENOTSUPP;
out:
...
mtp_complete_get_guid(struct usb_ep *ep, struct usb_request *req)
{
	...
	size = (req->actual >= sizeof(guid_info))
	    ? sizeof(guid_info)-1:req->actual;
	...
	if (ctrl->bRequestType ==
		...
		if (value >= 0) {
			...
			if (usb_ep_queue(cdev->gadget->ep0,
				    cdev->req, GFP_ATOMIC) < 0)
				DEBUG_MTPB("[%s:%d] setup queue error\n",
					__func__, __LINE__);
		}
		...
		if ((ctrl->bRequest == 1 || ctrl->bRequest == 0x54 ||
			...
			if (value >= 0) {
				...
				if (usb_ep_queue(cdev->gadget->ep0,
					    cdev->req, GFP_ATOMIC) < 0)
					DEBUG_MTPB(
						"[%s:%d] setup queue error\n",
						__func__, __LINE__);
			}
			return value;
		} else if ((ctrl->bRequest == 0xA3)
			&& (!(ctrl->bRequestType & USB_DIR_IN))) {

			DEBUG_MTPB("mtp: [%s] RECEIVE PC GUID / line[%d]\n",
						__func__, __LINE__);
			...
			value = usb_ep_queue(cdev->gadget->ep0,
						req, GFP_ATOMIC);
			if (value < 0)
				DEBUG_MTPB("[%s:%d]Error usb_ep_queue\n",
							__func__, __LINE__);
			else
			DEBUG_MTPB("[%s] ep-queue-sucecc line[%d]\n",
				__func__, __LINE__);
		}
		DEBUG_MTPB("mtp: mtp_ctrlrequest %02x.%02x v%04x i%04x l%u\n",
...
static int mtp_setup(void)
{
	...
	if (!mtpg) {
		DEBUG_MTPB("mtpg_dev_alloc memory failed\n");
		return -ENOMEM;
	}
	...
	if (!mtpg->wq) {
		DEBUG_MTPB("mtpg_dev_alloc work queue creation failed\n");
		rc =  -ENOMEM;
		goto err_out;
	}
	...
	if (rc != 0) {
		DEBUG_MTPB(" misc_register of mtpg Failed\n");
		goto err_out;
	}
	err = device_create_file(mtpg_device.this_device, &dev_attr_guid);
	DEBUG_MTPB("mtp: %s %s to create guid attr\n",
...
err_out:
	the_mtpg = NULL;
	kfree(mtpg);
	DEBUG_MTPB("mtp gadget driver failed to initialize\n");
	return rc;
}
