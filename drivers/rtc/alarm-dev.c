...
#endif
			__pm_wakeup_event(&info->vbus_wake_lock, HZ * 5);
		}
		break;
	case CABLE_TYPE_USB:
	case CABLE_TYPE_AC:
...
static void sec_bat_polling_work(struct work_struct *work)
{
	...
	if (info->initial_check_count) {
		schedule_delayed_work(&info->polling_work, 1000);
...
static __devinit int sec_bat_probe(struct platform_device *pdev)
{
	...
	wakeup_source_init(&info->vbus_wake_lock, 
		       "vbus_present");
	wakeup_source_init(&info->monitor_wake_lock, 
		       "sec-battery-monitor");
	wakeup_source_init(&info->cable_wake_lock, 
		       "sec-battery-cable");
